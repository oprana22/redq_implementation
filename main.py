...
def main():
    ...
    # 1. Setup the CSV Logger
    csv_filename = f"{env_name}_redq_results.csv"

    # Write the header to the CSV file before we start the loops
    with open(csv_filename, mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(["Environment", "Seed", "Step", "Average_Return"])

    # 2. Define the seeds we want to test for statistical significance
    seeds = [10, 20, 30]

    # 3. Wrap the training process in a loop for each seed
    for seed in seeds:
        print(f"\n==============================================")
        ...
        for step in range(max_steps):
            ...
            if (step + 1) % eval_interval == 0:
                ...
                # Optionally save your weights at each evaluation
                # agent.save_weights(f"{env_name}_seed_{seed}_weights.pth")
