...
def run_single_seed(seed):
    ...
    csv_filename = f"{env_name}_SAC_results.csv"
    ...
    agent = REDQ(obs_dim=obs_dim, act_dim=act_dim, act_limit=act_limit, N=2)

    # REDQ parameters (restored to paper standard for science run)
    start_steps = 5000  # warm up steps
    utd_ratio = 1  # neural network updates per agent step in the env
    max_steps = 200000  # Total steps for the academic run
